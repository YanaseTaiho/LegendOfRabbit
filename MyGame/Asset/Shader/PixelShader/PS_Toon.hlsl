...
Texture2D<float4> ToonTexture : register(t2);
...
float4 main(VS_OUTPUT input) : SV_Target
{
	...
	if (UseLight == 1)
	{
		// 平行光源
		{
			...
			float facing = -dot(normal.xyz, eyeDir);
			float2 toonTexcoord = float2(lightDiffuseFactor, facing);
			// スペキュラ計算
			...
			// ディフューズ
			outColor *= lightDiffuseFactor * DirLight.diffuse * Material.bump * ToonTexture.Sample(SamplerLinear, toonTexcoord);
			// アンビエント
